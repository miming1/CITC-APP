...
import AdminAuthModal from "../Admin Components/ActionAuthentication";
...
const updateRequirement = (index: number, value: string) => {
  ...
  updated[index].requirement_name = value;
...
const handleSubmit = async (): Promise<boolean> => {
  ...
  const cleanedRequirements = requirements
    .filter(
      (r) => r.requirement_name.trim().length > 0
    )
    .map((r) => ({
  ...
  const cleanedSteps = steps
    .filter(
      (s) => s.step_description.trim().length > 0
    )
    .map((s, i) => ({
      step_number: i + 1,
    ...
    <View style={s.overlay}>
      ...
      >
        ...
        <View style={s.tabRow}>
          ...
          >
            <Text style={[s.tabText, { color: theme.text }]}>
              Procedure
            </Text>
          </TouchableOpacity>
        ...
        <ScrollView style={s.scroll} showsVerticalScrollIndicator={true}>
          {activeTab === "procedure" ? (
            <>
              ...
              <Text style={[s.helperText, { color: theme.icon }]}>
                Mark a requirement as a document if it needs office tracking
                such as signatures, approvals, verification, or status updates.
              </Text>

              {requirements.map((req, index) => (
                <View key={index} style={s.requirementCard}>
                  <View style={s.rowItem}>
                    <TextInput
                      value={req.requirement_name}
                      onChangeText={(text) =>
                        updateRequirement(index, text)
                      }
                  ...
                  <TouchableOpacity
                    style={s.checkboxRow}
                    onPress={() =>
                      toggleRequirementDocument(index)
                    }
                  >
                    <View
                      style={[
              ...
              {steps.map((step, index) => (
                ...
                >
                  ...
                  <TextInput
                    ...
                    onChangeText={(text) =>
                      updateStep(index, "step_description", text)
                    }
                  ...
                  <TextInput
                    ...
                    onChangeText={(text) =>
                      updateStep(index, "office_location", text)
                    }
                  ...
                  <TextInput
                    ...
                    onChangeText={(text) =>
                      updateStep(index, "reference_link", text)
                    }
            ...
            <>
              ...
              >
                ...
                >
                  Individual FAQs can't be added here. Once this process is
                  created, a matching FAQ category will appear on the FAQ page
                  where you can add questions and answers for it.
                </Text>
              </View>
              ...
              <TextInput
                ...
                placeholder={
                  procedureName || "Defaults to the procedure name"
                }
                placeholderTextColor={theme.icon}
                style={[
                  s.input,
